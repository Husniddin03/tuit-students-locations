...
<div class="relative inline-flex" x-data="{ open: false }">
    <button
        ...
        <div
            class="absolute top-0 right-0 w-2.5 h-2.5 {{ $messengs->count() > 0 ? 'bg-red-500' : '' }} border-2 border-gray-100 dark:border-gray-900 rounded-full">
        </div>
    </button>
    <div class="origin-top-right z-10 absolute top-full -mr-48 sm:mr-0 min-w-80 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 py-1.5 rounded-lg shadow-lg overflow-hidden mt-1 {{ $align === 'right' ? 'right-0' : 'left-0' }}"
        ...
        <div class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase pt-1.5 pb-2 px-4">Xabarlar
        </div>
        <ul>
            @foreach ($messengs as $messeng)
                <li class="border-b border-gray-200 dark:border-gray-700/60 last:border-0">
                    <a class="block py-2 px-4 hover:bg-gray-50 dark:hover:bg-gray-700/20"
                        href="{{ route('students.show', $messeng->student->id) }}" @click="open = false"
                        ...
                        <span
                            class="block text-xs font-medium text-gray-400 dark:text-gray-500">{{ $messeng->created_at }}</span>
                    </a>
                </li>
            @endforeach
        </ul>
